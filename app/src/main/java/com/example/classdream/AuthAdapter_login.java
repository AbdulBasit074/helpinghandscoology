...
public class AuthAdapter_login extends FragmentStatePagerAdapter
        implements AuthFragment_login.Callback {

  private final AnimatedViewPager_login pager;
  private final SparseArray<AuthFragment_login> authArray;
  private final List<ImageView> sharedElements;
  private final ImageView authBackground;
  private float factor;

  AuthAdapter_login(FragmentManager manager,
                    AnimatedViewPager_login pager,
  ...
  @Override
  public AuthFragment_login getItem(int position) {
    AuthFragment_login fragment = authArray.get(position);
    if (fragment == null) {
      fragment = position != 1 ? new LogInFragment_login() : new SignUpFragment_login();
  ...
  @Override
  public void show(AuthFragment_login fragment) {
  ...
  private float getPageOffsetX(AuthFragment_login fragment) {
